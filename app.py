...
st.set_page_config(page_title="BlastTap 7.0 Master (반응속도·열수지·환원수지 통합)", layout="wide")
st.title("🔥 BlastTap 7.0 Master — AI 고로조업 통합 수지엔진 (반응·열·환원 통합보정)")
...
# ✅ 🔥 (신규) 반응속도·열수지·환원수지 동적 AI 보정항목 추가
st.sidebar.header("④ 반응속도·열수지·환원수지 추가입력")
...
if mode == "장입속도 기반 (자동)":
    ...
    charge_rate = reference_charge  # AI 보정 안전값 적용
...
temp_effect = 1 + ((hot_blast_temp - 1100) / 100) * 0.03  # 🔥 풍온 열수지 보정
pci_effect = 1 + (pci_rate - 150) / 100 * 0.02  # 🔥 PCI 미분탄 보정
iron_rate_effect = iron_rate / 9.0  # 🔥 선철 생성속도 보정
